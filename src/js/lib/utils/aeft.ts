...
const getLatestFile = (dir: string, suffix: string): string | null => {
    const getModified = (filePath: string) => fs.statSync(filePath).mtime.valueOf();
    ...
    fs.readdirSync(dir)
        ...
        .map(file => {
            if (
                latestFile === null ||
                getModified(path.join(dir, file)) > getModified(path.join(dir, latestFile))
            ) {
                latestFile = file;
            }
        });
...
export const getPrefsDir = (): string => {
    ...
    const mainDir = platform == "darwin" ? `${env.HOME}/Library/Preferences` : env.APPDATA || "";
...
export const getRenderSettingsList = (): string[] => {
    ...
    if (renderPref) {
        ...
        if (lines) {
            const firstLine = lines.findIndex(line => line.includes("Render Settings List"));
            const lastLine = lines.findIndex(line => line.includes("Still Frame RS Index"));
