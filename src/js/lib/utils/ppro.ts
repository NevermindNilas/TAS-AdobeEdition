...
const readDirSafe = (dir: string) => (fs.existsSync(dir) ? fs.readdirSync(dir) : []);
...
export const getAllLuts = (): { creative: string[]; technical: string[] } => {
    ...
    const appLutsDir = path.join(isWin ? appPath : path.dirname(appPath), "Lumetri", "LUTs");

    const winLocal = path.join(os.homedir(), "AppData", "Roaming", "Adobe", "Common", "LUTs");
    ...
    const macGlobal = path.join("Library", "Application Support", "Adobe", "Common", "LUTs");

    const appCreative = path.join(appLutsDir, "Creative");
    const appTechnical = path.join(appLutsDir, "Technical");
    const localCreative = isWin ? path.join(winLocal, "Creative") : path.join(macLocal, "Creative");
    ...
    const creative = [...appCreativeLuts, ...localCreativeLuts, ...globalCreativeLuts]
        .sort((a, b) => a.toLowerCase().localeCompare(b.toLowerCase()))
        .map(lut => path.basename(lut, path.extname(lut)));
    const technical = [...appTechnicalLuts, ...localTechnicalLuts, ...globalTechnicalLuts]
        .sort((a, b) => a.toLowerCase().localeCompare(b.toLowerCase()))
        .map(lut => path.basename(lut, path.extname(lut)));
