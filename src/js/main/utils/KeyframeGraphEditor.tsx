...
import { Flex, View, Button, Text, Heading, TextField, Tooltip, TooltipTrigger, Divider, ActionButton, AlertDialog, DialogTrigger, SearchField, Badge, Checkbox, Tabs, TabList, TabPanels, Item, Switch } from "@adobe/react-spectrum";
...
type PresetCurve = { label: string; bezier: string; tooltip: string; category: string };

const presetCurves: PresetCurve[] = [
  { label: "Ease", bezier: "0.25,0.1,0.25,1", tooltip: "Standard ease", category: "Standard" },
  { label: "Linear", bezier: "0,0,1,1", tooltip: "Linear transition", category: "Standard" },
  { label: "Ease In", bezier: "0.42,0,1,1", tooltip: "Ease in", category: "Standard" },
  { label: "Ease Out", bezier: "0,0,0.58,1", tooltip: "Ease out", category: "Standard" },
  { label: "Ease In Out", bezier: "0.42,0,0.58,1", tooltip: "Ease in out", category: "Standard" },
  
  { label: "Sine In", bezier: "0.47,0,0.745,0.715", tooltip: "Sine ease in", category: "Math" },
  { label: "Sine Out", bezier: "0.39,0.575,0.565,1", tooltip: "Sine ease out", category: "Math" },
  { label: "Sine In Out", bezier: "0.445,0.05,0.55,0.95", tooltip: "Sine ease in out", category: "Math" },
  { label: "Quad In", bezier: "0.55,0.085,0.68,0.53", tooltip: "Quadratic ease in", category: "Math" },
  { label: "Quad Out", bezier: "0.25,0.46,0.45,0.94", tooltip: "Quadratic ease out", category: "Math" },
  { label: "Quad In Out", bezier: "0.455,0.03,0.515,0.955", tooltip: "Quadratic ease in out", category: "Math" },
  { label: "Cubic In", bezier: "0.55,0.055,0.675,0.19", tooltip: "Cubic ease in", category: "Math" },
  { label: "Cubic Out", bezier: "0.215,0.61,0.355,1", tooltip: "Cubic ease out", category: "Math" },
  { label: "Cubic In Out", bezier: "0.645,0.045,0.355,1", tooltip: "Cubic ease in out", category: "Math" },
  { label: "Quart In", bezier: "0.895,0.03,0.685,0.22", tooltip: "Quartic ease in", category: "Math" },
  { label: "Quart Out", bezier: "0.165,0.84,0.44,1", tooltip: "Quartic ease out", category: "Math" },
  { label: "Quart In Out", bezier: "0.77,0,0.175,1", tooltip: "Quartic ease in out", category: "Math" },
  { label: "Quint In", bezier: "0.755,0.05,0.855,0.06", tooltip: "Quintic ease in", category: "Math" },
  { label: "Quint Out", bezier: "0.23,1,0.32,1", tooltip: "Quintic ease out", category: "Math" },
  { label: "Quint In Out", bezier: "0.86,0,0.07,1", tooltip: "Quintic ease in out", category: "Math" },
  { label: "Expo In", bezier: "0.95,0.05,0.795,0.035", tooltip: "Exponential ease in", category: "Math" },
  { label: "Expo Out", bezier: "0.19,1,0.22,1", tooltip: "Exponential ease out", category: "Math" },
  { label: "Expo In Out", bezier: "1,0,0,1", tooltip: "Exponential ease in out", category: "Math" },
  { label: "Circ In", bezier: "0.6,0.04,0.98,0.335", tooltip: "Circular ease in", category: "Math" },
  { label: "Circ Out", bezier: "0.075,0.82,0.165,1", tooltip: "Circular ease out", category: "Math" },
  { label: "Circ In Out", bezier: "0.785,0.135,0.15,0.86", tooltip: "Circular ease in out", category: "Math" },

  { label: "Back", bezier: "0.68,-0.55,0.27,1.55", tooltip: "Back ease", category: "Special" },
  { label: "Bounce", bezier: "0.68,-0.6,0.32,1.6", tooltip: "Bounce ease", category: "Special" },
  { label: "Elastic", bezier: "0.87,-0.41,0.19,1.44", tooltip: "Elastic ease", category: "Special" },
  { label: "Sharp In", bezier: "0.7,0,0.84,0", tooltip: "Very sharp ease in", category: "Special" },
  { label: "Sharp Out", bezier: "0.16,1,0.3,1", tooltip: "Very sharp ease out", category: "Special" },
  
  { label: "Custom 1", bezier: "0.33,1,0.68,1", tooltip: "Custom fast out slow in", category: "Custom" },
  { label: "Custom 2", bezier: "0.5,-0.5,0.5,1.5", tooltip: "Custom overshoot", category: "Custom" }
];
...
export default function KeyframeGraphEditor() {
  ...
  const [selectedTab, setSelectedTab] = useState<string>(() => {
      return localStorage.getItem('keyframe-selected-tab') || "Standard";
  });

  const handleTabChange = (key: React.Key) => {
      const keyString = String(key);
      setSelectedTab(keyString);
      localStorage.setItem('keyframe-selected-tab', keyString);
  };
  ...
  const draw = useCallback(() => {
    ...
    if (isHovering !== null) {
      const preset = [...presetCurves, ...customPresets].find(p => p.label === isHovering);
      if (preset) {
        const [gx1, gy1, gx2, gy2] = preset.bezier.split(',').map(Number);
        const gStart = bezierToCanvas(0, 0);
        const gEnd = bezierToCanvas(1, 1);
        const gCp1 = bezierToCanvas(gx1, gy1);
        const gCp2 = bezierToCanvas(gx2, gy2);

        ctx.beginPath();
        ctx.moveTo(gStart.x, gStart.y);
        ctx.bezierCurveTo(gCp1.x, gCp1.y, gCp2.x, gCp2.y, gEnd.x, gEnd.y);
  ...
  useEffect(() => {
    ...
    const updateCanvasSize = () => {
      ...
      try {
        ...
        if (rect.width > 0) {
          const size = Math.max(300, Math.min(rect.width, 800));
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    try {
      ...
      let clampedBezierX = Math.max(0, Math.min(1, bezierCoords.bezierX));
      let bezierY = bezierCoords.bezierY;

      if (snapToGrid) {
        clampedBezierX = Math.round(clampedBezierX * 20) / 20;
  ...
  }, [dragging, draggedPoint, canvasSize, canvasToBezier, bezierToCanvas, point1, point2, snapToGrid]);
  ...
  const reverseCurve = useCallback(() => {
    const values = bezierInput.split(",").map(v => parseFloat(v.trim()));
    if (values.length === 4) {
      const [x1, y1, x2, y2] = values;
      const newX1 = Math.round((1 - x2) * 100) / 100;
      const newY1 = Math.round((1 - y2) * 100) / 100;
      const newX2 = Math.round((1 - x1) * 100) / 100;
      const newY2 = Math.round((1 - y1) * 100) / 100;
      setBezierInput(`${newX1}, ${newY1}, ${newX2}, ${newY2}`);
      setSelectedPreset(null);
    }
  }, [bezierInput]);
  ...
  const handleFileUpload = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const newPresets: PresetCurve[] = flowData.map((item: any) => {
          if (!item.name || !Array.isArray(item.value) || item.value.length !== 4) {
            return null;
          }
          return {
            label: item.name,
            bezier: item.value.join(','),
            tooltip: `Imported: ${item.name}`,
            category: "Custom"
          };
        }).filter((p): p is PresetCurve => p !== null);

        if (newPresets.length === 0) {
            generateToast(2, "No valid presets found in file");
            return;
        }

        setCustomPresets(prev => {
          const existingLabels = new Set(prev.map(p => p.label));
          const uniqueNew = newPresets.filter(p => !existingLabels.has(p.label));
          
          if (uniqueNew.length < newPresets.length) {
             generateToast(3, `Skipped ${newPresets.length - uniqueNew.length} duplicates`);
          }
          
          return [...prev, ...uniqueNew];
        });

        generateToast(1, `Imported ${newPresets.length} presets!`);
      } catch (error) {
        console.error(error);
        generateToast(2, "Failed to parse .flow file");
      }
    };
      ...
      <View borderWidth="thin" borderColor="dark" borderRadius="medium" padding="size-150">
        <Flex direction="column" gap={8}>
          <Flex direction="row" gap={8} alignItems="center" justifyContent="space-between">
            ...
            <Switch isSelected={snapToGrid} onChange={setSnapToGrid}>
                Snap to Grid
            </Switch>
          </Flex>
          <Divider size="S" />

          <Flex direction="row" gap={8} alignItems="start" wrap>
            <Flex direction="column" gap={8} flexGrow={1} flexBasis={300}>
                <div
                ref={containerRef}
                style={{
                    flex: 1,
            ...
            <Flex direction="column" gap={6} flexGrow={1} flexBasis={200}>
              <Flex direction="column" gap={4}>
                <Text>Control Points</Text>
                <Flex direction="row" gap={4}>
                    <TextField 
                        label="P1 X" 
                        value={bezierInput.split(',')[0]?.trim()} 
                        onChange={(v) => {
                            const vals = bezierInput.split(',');
                            vals[0] = v;
                            setBezierInput(vals.join(','));
                        }}
                        width="auto"
                        flexGrow={1}
                    />
                    <TextField 
                        label="P1 Y" 
                        value={bezierInput.split(',')[1]?.trim()} 
                        onChange={(v) => {
                            const vals = bezierInput.split(',');
                            vals[1] = v;
                            setBezierInput(vals.join(','));
                    ...
                    <TextField 
                        label="P2 X" 
                        value={bezierInput.split(',')[2]?.trim()} 
                        onChange={(v) => {
                            const vals = bezierInput.split(',');
                            vals[2] = v;
                            setBezierInput(vals.join(','));
                    ...
                    <TextField 
                        label="P2 Y" 
                        value={bezierInput.split(',')[3]?.trim()} 
                        onChange={(v) => {
                            const vals = bezierInput.split(',');
                            vals[3] = v;
                            setBezierInput(vals.join(','));
              ...
              <Flex direction="row" gap={4} wrap justifyContent="center">
                <TooltipTrigger>
                ...
                <TooltipTrigger>
                    <ActionButton onPress={reverseCurve} isQuiet>
                        <Compare />
                    </ActionButton>
                    <Tooltip>Reverse Curve</Tooltip>
                </TooltipTrigger>
              </Flex>
          ...
          <Flex direction="column" gap={6}>
            <Flex direction="row" justifyContent="space-between" alignItems="center" gap={8} wrap>
                <SearchField
                ...
                <Flex direction="row" gap={4}>
                    <TooltipTrigger>
                        ...
                        <Tooltip>Import .flow presets</Tooltip>
                    </TooltipTrigger>
                    <input 
                        type="file" 
                        ref={fileInputRef} 
            ...
            <Tabs aria-label="Preset Categories" selectedKey={selectedTab} onSelectionChange={handleTabChange}>
                <TabList>
                    <Item key="Standard">Standard</Item>
                    <Item key="Math">Math</Item>
                    <Item key="Special">Special</Item>
                    <Item key="Custom">Custom</Item>
                </TabList>
                <TabPanels>
                    {["Standard", "Math", "Special", "Custom"].map(category => (
                        <Item key={category}>
                            <Flex wrap gap={4} justifyContent="start" marginTop="size-100">
                            {[...presetCurves, ...customPresets]
                                ...
                                <TooltipTrigger key={preset.label}>
                                    ...
                                    onMouseEnter={() => setIsHovering(preset.label)}
                                    onMouseLeave={() => setIsHovering(null)}
                                    style={{
                                        position: "relative",
                                        border: selectedPreset?.label === preset.label
                                        ? "2px solid #7cbdfa"
                                        : isHovering === preset.label
                                        ...
                                        transform: isHovering === preset.label ? "scale(1.05)" : "scale(1)",
                                    }}
                                    >
                                    {preset.category === "Custom" && isHovering === preset.label && (
                                        <div 
                                            style={{ position: "absolute", top: -6, right: -6, zIndex: 10 }} 
                                            onClick={(e) => e.stopPropagation()}
                                        >
                                            <DialogTrigger>
                                                <ActionButton isQuiet onPress={(e) => {}} UNSAFE_style={{ minWidth: '20px', width: '20px', height: '20px', padding: 0, borderRadius: '50%', backgroundColor: '#2a2a2a', border: '1px solid #444' }}>
                                                    <Delete size="S" />
                                                </ActionButton>
                                                <AlertDialog
                                                    title="Delete Preset"
                                    ...
                                    <svg width="48" height="38" viewBox="-5 -5 50 42" style={{ overflow: "visible" }}>
