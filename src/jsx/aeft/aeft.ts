...
export const removeDuplicateFrames = (): boolean => {
    try {
        if (!app.project?.activeItem || !(app.project.activeItem instanceof CompItem)) {
        ...
        app.beginUndoGroup("Remove Duplicate Frames");
        ...
        for (let i = 0; i < selected.length; i++) {
            ...
            const totalFrames = (outPoint - inPoint) / frameDuration;

            for (let j = 0; j < totalFrames - frameDuration; j++) {
                const curFrame: number[][] = [];
                let pos = 0;
                curTime = inPoint + frameDuration * j;
                comp.time = curTime;
                ...
                const wSp = width / acc / 2;
                for (let w = width / acc / 2; w < width; w += width / acc) {
                    const h = height / 2;

                    const colorSourceText = colorText.property("Source Text") as Property;
                    colorSourceText.expression = `targetLayer = thisComp.layer("${layer.name}"); samplePoint = [${w},${h}]; sampleRadius = [${wSp},${h}]; sampledColor = 255 * targetLayer.sampleImage(samplePoint, sampleRadius); r = Math.round(sampledColor[0]); g = Math.round(sampledColor[1]); b = Math.round(sampledColor[2]); outputString = r + "," + g + "," + b`;

                    const rgbString = (
                        colorText.property("Source Text") as Property
                    ).value.toString();
                ...
                if (j !== 0) {
                    const checkResult = checkRGB(curFrame, prevFrame, acc);
                    let samePixels = 0;

                    for (let x = 0; x < acc; x++) {
                        if (checkChunk(curFrame[x], prevFrame[x], x) === 0) {
                            samePixels++;
                        }
                    }

                    if (
                        (samePixels !== acc && checkResult >= threshold1) ||
                        (samePixels === acc && checkResult >= threshold1) ||
                        (skips >= 2 && checkResult >= threshold2)
                    ) {
                        timeRemapProperty.addKey(curTime);
            ...
            const property = timeRemapProperty;
            const numKeys = property.numKeys;

            if (numKeys > 0) {
                if (numKeys > 1 && property.keyTime(numKeys) === outPoint) {
                    property.removeKey(numKeys);
                }

                let timeAdd = frameDuration;
                let z = 2;

                if (numKeys > 0 && property.keyTime(1) === layer.inPoint) {
                    z--;
                }

                const currentNumKeys = property.numKeys;
                for (let keyIndex = z; keyIndex < currentNumKeys; keyIndex++) {
                    if (keyIndex + 1 <= property.numKeys) {
                        const key = property.keyValue(keyIndex + 1);
                        property.removeKey(keyIndex + 1);
                        property.setValueAtTime(inPoint + timeAdd, [key]);
                        timeAdd += frameDuration;
                    }
                }

                const finalNumKeys = property.numKeys;
                if (finalNumKeys > 0 && property.keyTime(finalNumKeys) === layer.outPoint) {
                    const key = property.keyValue(finalNumKeys);
                    property.removeKey(finalNumKeys);
                    property.setValueAtTime(inPoint + timeAdd, [key]);
                    timeAdd += frameDuration;
                }

                layer.outPoint = inPoint + timeAdd;
            }
...
export const removeDuplicateFramesImproved = (
